"""Static stock watchlist.

Crypto no longer lives here — it's fetched live from Bitkub's own API every
run (see bitkub_source.get_universe()), so "every coin on Bitkub" always
reflects Bitkub's current listings without needing manual updates here.

Stocks don't have an equivalent free live-screener API, so this list is
built from two sources and hardcoded:

  - US_STOCK_NAMES: the full S&P 500 constituent list, pulled from the public
    dataset at github.com/datasets/s-and-p-500-companies (snapshot taken
    2026-08-09). Covers effectively all large-cap US stocks tradable via Dime.
  - TH_STOCK_NAMES: ~90 liquid Thai SET-listed stocks (SET50 + notable SET100
    names), manually compiled and each individually verified against Yahoo
    Finance's `.BK` chart endpoint. This is NOT a live feed of the official
    SET100 index (no free API exposes that) — edit this dict directly to
    add/remove names.
"""
...
TH_STOCK_NAMES = {
    ...
    "JMART.BK": "Jaymart Group Holdings",
    "JMT.BK": "JMT Network Services",
    ...
    "STGT.BK": "Sri Trang Gloves (Thailand)",
    "SUPER.BK": "Super Energy Corporation",
    "SYNEX.BK": "Synnex (Thailand)",
    "TCAP.BK": "Thanachart Capital",
    "TFG.BK": "Thaifoods Group",
    "THANI.BK": "Ratchthani Leasing",
    "THG.BK": "Thonburi Healthcare Group",
    "TIDLOR.BK": "Tidlor Holdings",
    "TISCO.BK": "TISCO Financial Group",
    "TKN.BK": "Taokaenoi Food & Marketing",
    "TOA.BK": "TOA Paint (Thailand)",
    "TOP.BK": "Thai Oil",
    "TPIPP.BK": "TPI Polene Power",
    "TRUE.BK": "True Corporation",
    "TTA.BK": "Thoresen Thai Agencies",
    "TTB.BK": "TMBThanachart Bank",
    "TU.BK": "Thai Union Group",
    "TVO.BK": "Thai Vegetable Oil",
    "VGI.BK": "VGI",
    "WHA.BK": "WHA Corporation",
    "WHAUP.BK": "WHA Utilities and Power",
